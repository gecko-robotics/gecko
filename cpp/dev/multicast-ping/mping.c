...
void send_packet(struct mping_struct *packet) {
  ...
  pkt_len = sizeof(struct mping_struct);
    // pkt_len = sizeof(*packet);

  /* send string to multicast address */
  if ((sendto(sock, packet, pkt_len, 0,
       (struct sockaddr *) &mc_addr,
       sizeof(mc_addr))) != pkt_len) {
...
void sender_listen_loop() {
  ...
  while (1) {
    ...
    if ((recv_len = recvfrom(sock, recv_packet, MAX_BUF_LEN,
         0, NULL, 0)) < 0) {
      if (errno == EINTR) {
        /* interrupt is ok */
        continue;
...
int process_mping_packet(char *packet, int recv_len,
                         unsigned char type) {

  /* validate packet size */
  if (recv_len < sizeof(struct mping_struct)) {
    // if (verbose) printf("Discarding packet: too small (%uz)\n",
    //                     strlen(packet));
    return(-1);
  }
  ...
  if ((rcvd_pkt->version_major != VERSION_MAJOR) ||
    ...
    if (verbose) printf("Discarding packet: version mismatch (%d.%d)\n",
...
void usage() {
  ...
  printf("-p ttl       Multicast Time-To-Live to send,\n");
