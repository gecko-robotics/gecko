...
void SSocket::init(string mc_addr_str, uint16_t mc_port, uint8_t mc_ttl, bool reuse){
    ...
    if (true) {
        int yes = 1;
        int err = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (int*) &yes, sizeof(yes));
        if (err < 0){
    ...
    int one = 0;  // 0-disable  1-enable
    if ((setsockopt(sock, IPPROTO_IP, IP_MULTICAST_LOOP,
       (int*) &one, sizeof(one))) < 0) {
        // perror("SSocket::init setsockopt() IP_MULTICAST_TTL failed");
        // exit(1);
    ...
    struct sockaddr_in aaddr = create_sockaddr(mc_port, INADDR_ANY);
